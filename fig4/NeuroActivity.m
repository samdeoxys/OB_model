...
%    Iext = Respiration(tt) .* Iext_max(:,tt);
    ...
    % [Sam] nicotinic modulation current
    Imodmit(:) = param.gModmit.*(param.EModmit - Vmit(:,tt-1));
    Imodmit_matrix(:,tt) = Imodmit(:);
    ...
    Vmit(:,tt) = Vmit(:,tt - 1) + (param.dt ./ taumit(:)) .* ...
        ((Iext(:) + Igradistmit(:) + Igraspmit(:) + Imodmit(:)) - Vmit(:,tt - 1) + Restmit(:) + Vnoisemit);
     ...
     if param.ProximalON == true
         ...
         if param.PCinputON == true && param.ProximalON == true % set PC input to graprox 
            ...
            elseif strcmp(param.PCtype,'constant') == true
                Ipcgraprox = wpcinput * (param.PCparam1 * ones(param.nGraprox,1) +  + param.Inoise*randn(param.nGraprox,1)); % weight diagonal matrix dot column vector of strength
                
            elseif strcmp(param.PCtype,'sine') == true
                Ipcgraprox = wpcinput * (ones(param.nGraprox,1) + param.Inoise*randn(param.nGraprox,1)) +param.PCparam1 * sin(2*pi*param.PCparam3/(1000/param.dt)*tt); %[Sam] PCparam2 is weight coded in wpcinput, PCparam1 is amplitude for the fluctuation, PCparam3 is frequnecy;
...
if param.MitFrac<1 % for part of the mitral cells to be completely in sync
    syncinds = randsample(1:param.nMitral,floor(param.nMitral*param.MitFrac));
    W(syncinds) = 0.001*rand + param.Wmin;
    nonsyncinds = setdiff(syncinds,1:param.nMitral);
end



if param.flagRespiration == false %[[Sam]]
    if param.MitFrac ==1 % no that mitrals with the same input
        ...
        Iext(nonsyncinds,timerange) = repmat(W(nonsyncinds) .* (param.extparam + param.Inoise*randn(length(nonsyncinds),1)),1,length(timerange)); %[[Sam]] 
        Iext(syncinds,timerange) = repmat(W(syncinds).*param.extparamFrac,1,length(timerange)); % [Sam] the mitrals with same input has no noise
    end
else %[[Sam]]

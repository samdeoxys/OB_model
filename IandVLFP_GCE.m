...
%         Gradistmit_filtered(n,:) = filtfilt(Hd.Numerator,1,InputCurrent.Igradistmit(n,:));
        Gradistmit_filtered(n,:) = InputCurrent.Igradistmit(n,:); %+ InputCurrent.Igraspmit(n,:); %[[Sam]] updated to include spike dependent gaba
...
%         MitGradist_filtered(n,:) = filtfilt(Hd.Numerator,1,GraDistal(n).V);
    ...
    MitLFPs.extra = abs(GraDistLFPs.MitGradistGlobal) + abs(MitLFPs.GradistMitGlobal); %[[Sam]]? thought lfp should be the sum of both IPSC and EPSC;  
